...
class File:
    def __init__(self):
        ...
        self.open_path = fr'check_result'
        self.rawdata_path = fr'raw_data'
    
    def citrix_to_file(self, all_data, result_data, vendor):
        open_path = fr'{self.open_path}/{vendor}_{self.today_date}.csv'
        file = open(open_path, 'w', encoding='cp949')
        file.write("장비정보, 장비정보, 장비정보, Resource, Resource, Resource, Interface, Interface, Interface, Interface, Interface, Interface, Interface, SLB Info, SLB Info, SLB Info, SLB Info, SLB Info\n")
    ...
    def cisco_to_file(self, all_data, result_data, vendor):
        open_path = fr'{self.open_path}/{vendor}_{self.today_date}.csv'
        file = open(open_path, 'w', encoding='cp949')
        file.write("장비정보, 장비정보, 장비정보, Resource, Resource, Resource, Power, Power, Power, Temperature, Temperature, Temperature, Temperature, FAN, FAN, FAN, FAN, FAN, FAN, Interface, Interface, Interface, Interface, Interface, Interface, Interface\n")
        file.write("Hostname, IP, Model, CPU_Use, MEM_Use, 결과, Power_1A, Power_1B, 결과, Inlet, Outlet, Hotspot, 결과, FAN1, FAN2, FAN3, FAN4, FAN5, 결과, 1G_up, 1G_down, 10G_up, 10G_down, ch_up, ch_down, 결과\n")
        for data in all_data:
            for result in result_data:
                if data['Hostname'] == result['hostname']:
                    if 'FAN3' in data:
    ...
    def citrix_raw_data(self, all_data):
        open_path = fr'{self.rawdata_path}/citrix/citrix_raw_data_{self.today_date}.csv'
        file = open(open_path, 'w', encoding='cp949')
        for data in all_data:
            file.write(
    ...
    def cisco_raw_data(self, all_data):
        open_path = fr'{self.rawdata_path}/cisco/cisco_raw_data_{self.today_date}.csv'
        file = open(open_path, 'w', encoding='cp949')
        for data in all_data:
            if 'FAN3' in data:
                file.write(
