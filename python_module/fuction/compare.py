...
class CompareResult:
    def __init__(self):
        ...
        self.interface = dict()
        self.slbinfo = dict()
        self.path = fr'raw_data'
        self.today_date = now.strftime("%Y-%m-%d")
        self.yesterday_date = (now - timedelta(day=1)).strftime("%Y-%m-%d")
        
    def citrix_compare(self, all_data):
        all_result = []
        with open(f'{self.path}/citrix/citrix_raw_data_{self.yesterday_date}.csv', 'r') as file:
        ...
        for data in all_data:
            for y_data in y_data_line:
                ...
                if data['Hostname'] == hostname:
                    ...
                    y_vserver = data_word[11].split()
                    y_service = data_word[13].split()
                    ## CPU 비교 
                    ...
                    if t_interface_sum != y_interface_sum:
                        ...
                        interface_result = '정상'
                    ## slb 비교
                    t_vserver = str(data['Vserver_up']).split()
                    t_service = str(data['Service_up']).split()
                    if t_vserver != y_vserver or t_service != y_service:
                        slbinfo_result = '점검필요'
                    else:
                        slbinfo_result = '정상'
                    ## 비교결과 저장
    ...
    def cisco_compare(self, all_data):
        all_result = []
        with open(f'{self.path}/cisco/cisco_raw_data_{self.yesterday_date}.csv', 'r') as file:
            yesterday_data = file.read()
        y_data_line = [line for line in yesterday_data.splitlines()]
        for data in all_data:
            for y_data in y_data_line:
                ...
                if data['Hostname'] == hostname:
                    ...
                    if t_temp_inlet > 40 or temp_inlet_change >=10 or t_temp_outlet > 45 or temp_outlet_change >=10 or t_temp_hotspot > 60 or temp_outlet_change >=10:
                        temp_result = '점검필요'
                    else:
                        temp_result = '정상'
                    ## FAN상태 비교
